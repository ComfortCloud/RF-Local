	...
	char *pReaderHostName)
{
	...
	int flag = 0;
	std::thread th(getKeyboard, &flag);
			...
			if (0 == resetConfigurationToFactoryDefaults())
			{
				...
				if (0 == setImpinjReaderConfig())
				{
					...
					if (0 == addROSpec())
					{
						...
						{
							rc = 6;
							std::vector<EPC> dataset;
							while (!flag)
							{
								if (0 == startROSpec())
								{
									rc = 7;
									if (0 == awaitAndPrintReport(20)) // timeout(seconds)
									{
										rc = 8;
										if (0 == stopROSpec())
										{
								...
								// process ant3
								int cnt3 = 0;
								for (auto it3 = antDataVec3.begin(); it3 != antDataVec3.end(); it3++)
								{
									if (dataset.empty())
									{
										EPC epc_new;
										strcpy(epc_new.epc, (*it3).epc);
										epc_new.reader.readerID = 1;
										epc_new.reader.ant3.antennaId = 3;
										epc_new.reader.ant3.phase.push_back((*it3).phase);
										epc_new.reader.ant3.rssi.push_back((*it3).rssi);
										epc_new.reader.ant3.timestamp.push_back((*it3).timestamp_pc);
										dataset.push_back(epc_new);
									}
									else
									{
										...
										for (auto it = dataset.begin(); it != dataset.end(); it++)
										{
											if (strcmp((*it3).epc, (*it).epc) == 0)
											{
												searchFlag = 1;
												(*it).reader.ant3.antennaId = 3;
												(*it).reader.ant3.phase.push_back((*it3).phase);
												(*it).reader.ant3.rssi.push_back((*it3).rssi);
												(*it).reader.ant3.timestamp.push_back((*it3).timestamp_pc);
											}
											else
											{
												continue;
										...
										// std::cout << (*it3).epc << " " << (*it3).phase << " " <<std::fixed<< timeVec3[cnt3]<< " "<< (*it3).rssi << std::endl;
										cnt3++;
									}
								}
								...
								for (auto it = dataset.begin(); it != dataset.end(); it++)
								{
									...
									// pub data from ant1
									impinj_control::RFIDdata msg1;
									impinj_control::RFIDdata msg2;
									impinj_control::RFIDdata msg3;

									msg1.epc = (*it).epc;
									msg2.epc = (*it).epc;
									msg3.epc = (*it).epc;

									msg1.readerID = (*it).reader.readerID;
									msg2.readerID = (*it).reader.readerID;
									msg3.readerID = (*it).reader.readerID;

									msg1.antID = (*it).reader.ant1.antennaId;
									msg2.antID = (*it).reader.ant2.antennaId;
									msg3.antID = (*it).reader.ant3.antennaId;

									ROS_INFO("ant id: ant1: %d, ant2: %d, ant3: %d", msg1.antID, msg2.antID, msg3.antID);

									int cnt1 = 0;
									for (auto it1 = (*it).reader.ant1.phase.begin(); it1 != (*it).reader.ant1.phase.end(); it1++)
									{
										msg1.phase.push_back((*it1));
										msg1.rssi.push_back((*it).reader.ant1.rssi[cnt1]);
									...
									for (auto it2 = (*it).reader.ant2.phase.begin(); it2 != (*it).reader.ant2.phase.end(); it2++)
									{
										...
										msg2.rssi.push_back((*it).reader.ant2.rssi[cnt2]);
									...
									int cnt3 = 0;
									for (auto it3 = (*it).reader.ant3.phase.begin(); it3 != (*it).reader.ant3.phase.end(); it3++)
									{
										msg3.phase.push_back((*it3));
										msg3.rssi.push_back((*it).reader.ant3.rssi[cnt3]);
										msg3.timestamp.push_back((*it).reader.ant3.timestamp[cnt3]);
										cnt3++;
									}
									pub1.publish(msg3);
									printf("INFO: publish msg3 of tag %d finished\n", count);
									rate_20hz.sleep();

									count++;
								}

								// clear savings for next iteration
								antDataVec1.clear();
								timeVec1.clear();
								antDataVec2.clear();
								timeVec2.clear();
...
// 							{
// 								rc = 7;
// 								if (0 == awaitAndPrintReport(10)) // 30s
// 								{
// 									rc = 8;
// 									if (0 == stopROSpec())
// 									{
...
int CMyApplication::checkConnectionStatus(void)
{
	CMessage *pMessage;
...
**
*****************************************************************************/
int CMyApplication::enableImpinjExtensions(void)
{
...
int CMyApplication::resetConfigurationToFactoryDefaults(void)
{
...
**
*****************************************************************************/
int CMyApplication::getReaderCapabilities(void)
{
...
**
*****************************************************************************/
int CMyApplication::setImpinjReaderConfig()
{
	CSET_READER_CONFIG *pCmd;
	CMessage *pRspMsg;
	CSET_READER_CONFIG_RESPONSE *pRsp;
...
int CMyApplication::addROSpec(void)
{
...
int CMyApplication::enableROSpec(void)
{
	CENABLE_ROSPEC *pCmd;
	CMessage *pRspMsg;
...
int CMyApplication::startROSpec(void)
{
	CSTART_ROSPEC *pCmd;
	CMessage *pRspMsg;
...
int CMyApplication::stopROSpec(void)
{
	CSTOP_ROSPEC *pCmd;
	CMessage *pRspMsg;
...
int CMyApplication::awaitAndPrintReport(int timeout)
{
	int bDone = 0;
	int retVal = 0;
...
void CMyApplication::printTagReportData(
...
**
*****************************************************************************/
void CMyApplication::formatOneEPC(
...
**
*****************************************************************************/
int CMyApplication::getOnePhaseAngle(
...
**
*****************************************************************************/
int CMyApplication::getOnePeakRSSI(
...
**
*****************************************************************************/
int CMyApplication::getOneTimestamp(
...
**
*****************************************************************************/
int CMyApplication::getOneAntenna(
	CAntennaID *pAntenna,
	unsigned short *out)
{
	if (NULL != pAntenna)
...
**
*****************************************************************************/
int CMyApplication::getOneChannelIndex(
...
**
*****************************************************************************/
int CMyApplication::estimateVelocity(
	char *epcStr,
	double rssi,
	double phase,
...
**
*****************************************************************************/
void CMyApplication::printOneTagReportData(
	CTagReportData *pTagReportData)
{
	...
	// char * target_epc = "3008-33B2-DDD9-0140-0000-3000";
	...
	if (getOneTimestamp(pTagReportData->getLastSeenTimestampUTC(), &time))
	{
		...
		oneTagData.timestamp_pc = ros::Time::now().toSec();
	}
	...
	if (antenna == 1)
	{
		// leftdataArray[leftdataIndex++] = oneTagData;
		//double current_Time = ros::Time::now().toSec();
		//timeVec1.push_back(current_Time);
		antDataVec1.push_back(oneTagData);
	}

	if (antenna == 2)
	{
		// rightdataArray[rightdataIndex++] = oneTagData;
		//double current_Time = ros::Time::now().toSec();
		//timeVec2.push_back(current_Time);
		antDataVec2.push_back(oneTagData);
	}
	...
	if (antenna == 3)
	{
		// leftdataArray[leftdataIndex++] = oneTagData;
		//double current_Time = ros::Time::now().toSec();
		//timeVec3.push_back(current_Time);
		antDataVec3.push_back(oneTagData);
	}
	...
	if (antenna == 4)
	{
		// rightdataArray[rightdataIndex++] = oneTagData;
		//double current_Time = ros::Time::now().toSec();
		//timeVec4.push_back(current_Time);
		antDataVec4.push_back(oneTagData);
	}
}
...
void CMyApplication::handleReaderEventNotification(
	CReaderEventNotificationData *pNtfData)
{
...
void CMyApplication::handleAntennaEvent(
	CAntennaEvent *pAntennaEvent)
{
...
void CMyApplication::handleReaderExceptionEvent(
	CReaderExceptionEvent *pReaderExceptionEvent)
{
...
int CMyApplication::checkLLRPStatus(
...
int CMyApplication::sendMessage(
	CMessage *pSendMsg)
{
	CConnection *pConn = m_pConnectionToReader;
...
void CMyApplication::printXMLMessage(
	CMessage *pMessage)
{
	char aBuf[10 * 1024];
